...
${deliveryBrokerBlockItems}
  ...
  if (cronHealth) {
    if (cronHealth.status === 'unavailable') {
      cronHealthSection = `
<h2>Cron Health (inspection unavailable)</h2>
<p>${String(cronHealth.message || 'Cron inspection unavailable.').replace(/[<>]/g, '')}</p>`;
    } else if (Array.isArray(cronHealth.jobs) && cronHealth.jobs.length > 0) {
...
${cronRows}
    ...
    } else if (cronHealth.status === 'empty') {
      cronHealthSection = `
<h2>Cron Health (no enabled jobs)</h2>
<p>${String(cronHealth.message || 'No enabled cron jobs found.').replace(/[<>]/g, '')}</p>`;
    }
  }
