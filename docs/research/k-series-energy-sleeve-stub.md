...
**Existing target sum:** Approved Instruments table sums to **123%** today (verified 2026-06-04 — strategic rebalance buffer baked in across the existing 16-instrument set). The asset-class targets at the top of `portfolio.md` cap effective allocation at 100% (65% global equities + 20% Swiss + 15% bonds/cash), so the 123% on instruments is *aspirational ceilings*, not committed allocation. Adding 5–7% energy on top makes the instrument-target sum 128–130%; well within precedent. K2 will still need a sizing decision, but no allocation tension to resolve before adding rows.
