...
- Biggest remaining gaps: Interactive Brokers native read-only connectivity maturity, broker-backed instrument resolution/pricing, and live-gated execution.
...
| 12. Interactive Brokers adapter MVP | partial | Native TWS / IB Gateway socket connectivity and dry-run proposal plumbing exist, but current readiness is not durable enough to replace simulated holdings/pricing. Contract search currently returns no matches for the approved ETF symbols and broker-backed pricing is falling back to draft assumptions. |
...
4. Interactive Brokers instrument lookup and price lookup scaffolding now exist, but contract resolution and order-quote depth are still incomplete because contract-details searches are currently returning no usable matches for the approved ETF symbols and delayed market-data requests still fall back to draft pricing.
...
- Native IB Gateway live login, socket transport, and dry-run proposal plumbing are in place, but current readiness checks are failing again (`reason: native_error`), so simulated holdings/pricing remain the active safe path.
