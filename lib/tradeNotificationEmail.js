...
function enrichHoldingRow(row = {}, approvedInstruments = [], snapshotRows = []) {
  ...
  return {
    symbol: approved?.ibkrLocalSymbol || approved?.ibkrSymbol || row.symbol || approved?.tickerOrIsin || snapshot?.symbol || snapshot?.tickerOrIsin || '—',
    name: approved?.name || snapshot?.name || row.name || approved?.ibkrLocalSymbol || approved?.ibkrSymbol || row.symbol || 'Unnamed instrument',
    ...
    ibkrSymbol: approved?.ibkrSymbol || null,
    ibkrLocalSymbol: approved?.ibkrLocalSymbol || null,
  };
}
...
function buildNormalizedTradeContext(trade, portfolio, { portfolioDir = '' } = {}) {
  ...
  // Phase A: readiness flags
  const hasResultingTotalHeld = normalizedTrade.resultingTotalHeld != null;
  const hasTrustedPortfolioTotals = Number.isFinite(Number(normalizedPortfolio.totalValueChf)) && Number(normalizedPortfolio.totalValueChf) > 0
    && Number.isFinite(Number(normalizedPortfolio.cashChf));

  // Bridge ISIN <-> conid <-> ibkr_symbol via approvedInstruments so a trade
  // identified by ISIN can match a holdings row identified by conid or local
  // symbol (and vice versa). Strict-string equality on `symbol` alone missed
  // every IBKR-resolved holding (where the symbol column carries the conid).
  const tradeIdentityCandidates = [
    displaySymbol,
    trade.symbol,
    trade.tickerOrIsin,
    trade.name,
    ...
    normalizedTrade.name,
  ];
  const tradeApproved = findApprovedInstrument(tradeIdentityCandidates, approvedInstruments);
  const tradeIdentitySet = new Set(
    [
      ...tradeIdentityCandidates,
      ...(tradeApproved ? identityValues(tradeApproved) : []),
    ]
      .map(identityKey)
      .filter(Boolean),
  );
  const hasTrustedPortfolioHoldings = normalizedPortfolio.holdings.length > 0
    && normalizedPortfolio.holdings.some((row) => {
      if (row.quantityHeld == null || row.valueChf == null) return false;
      const rowKeys = identityValues(row).map(identityKey).filter(Boolean);
      return rowKeys.some((key) => tradeIdentitySet.has(key));
    });
  const portfolioAfterFillReady = hasTrustedPortfolioTotals && hasTrustedPortfolioHoldings;
  const investorEmailReady = hasResultingTotalHeld && portfolioAfterFillReady;
...
function buildTradeEmailHtml(trade, portfolio, openOrders = [], options = {}) {
  ...
  const purchaseRows = [
    ...
    ...(hasDistinctCommissionRow(context.investorTrade)
      ...
      : []),
    // Phase C: only show Resulting total held when it is actually known
    ...(context.readiness.hasResultingTotalHeld
      ? [{ label: 'Resulting total held', value: String(context.investorTrade.resultingTotalHeld) }]
      : []),
  ];
  ...
  // Phase C: only render portfolio-after-fill when the snapshot is trusted
  const portfolioCard = context.readiness.portfolioAfterFillReady
    ? card({
...
function buildTradeEmailText(trade, portfolio, openOrders = [], options = {}) {
  ...
  // Phase C: only show Resulting total held when known
  if (context.readiness.hasResultingTotalHeld) {
    lines.push(`Resulting total held: ${context.investorTrade.resultingTotalHeld}`);
  }
  const afterFillFacts = buildAfterFillFacts(context, openOrders);
  ...
  // Phase C: only render portfolio-after-fill when trusted
  if (context.readiness.portfolioAfterFillReady) {
    lines.push('', 'Portfolio after fill');
    lines.push(`Portfolio value: CHF ${Number(context.portfolio.totalValueChf).toFixed(2)}`);
    lines.push(`Cash balance: CHF ${Number(context.portfolio.cashChf).toFixed(2)}`);
