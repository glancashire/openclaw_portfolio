 ...
 *     -> Promise<event>
 *   resolveIssue({ issueId, statusDetails, fetchImpl })
 *     -> Promise<issue>  (status:'resolved')
...
/**
 * resolveIssue — mark a Sentry issue as resolved.
 *
 * Requires `event:admin` scope on the auth token. Optionally accepts
 * statusDetails for `inNextRelease` / `inRelease` / `inCommit` flavors;
 * default behavior is plain resolution.
 */
async function resolveIssue(opts) {
  const { issueId, statusDetails, fetchImpl } = opts || {};
  if (!issueId) throw new Error('resolveIssue requires { issueId }');
  const url = `${_base()}/issues/${encodeURIComponent(issueId)}/`;
