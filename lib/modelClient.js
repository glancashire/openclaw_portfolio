...
function callOpenclaw({ system, user, maxTokens: _mt, timeoutMs, openclawBin, cfg }) {
  ...
  if (Number.isFinite(Number(_mt))) args.push('--max-tokens', String(Math.max(1, Math.floor(Number(_mt)))));
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      if (settled) return;
      if (code !== 0) {
        return finish(reject, Object.assign(new Error(`openclaw exited ${code}: ${stderr.trim().slice(0, 200) || stdout.trim().slice(0, 200)}`), { code: 'CLI_ERROR', exitCode: code }));
      }
      let parsed;
...
async function callAnthropic({ system, user, maxTokens, env, fetchImpl, cfg, signal }) {
  const body = { model: cfg.anthropicModel, max_tokens: maxTokens, system: system || undefined, messages: [{ role: 'user', content: user }] };
