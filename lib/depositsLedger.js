...
/**
 * Returns cumulative net-deposited CHF as-of (inclusive) the given ISO date.
 * Walks the ledger entries (deposits add, withdrawals subtract) where
 * `entry.date <= asOfIsoDate`. Both inputs are ISO `YYYY-MM-DD`.
 *
 * Returns 0 when the ledger is empty or no entries fall on/before the date.
 */
function netDepositedAsOf(entries, asOfIsoDate) {
  if (!Array.isArray(entries) || !entries.length) return 0;
  if (typeof asOfIsoDate !== 'string' || !asOfIsoDate) return 0;
  let net = 0;
  for (const e of entries) {
    if (typeof e.date !== 'string') continue;
    if (e.date > asOfIsoDate) continue;
    const amount = Number(e.amountChf);
    if (!Number.isFinite(amount)) continue;
    if (e.direction === 'deposit') net += amount;
    else if (e.direction === 'withdrawal') net -= amount;
  }
