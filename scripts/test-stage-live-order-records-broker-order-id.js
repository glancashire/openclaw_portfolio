...
async function main() {
  ...
  try {
    ...
    assert(result.ok, `Expected live staging result ok, got ${JSON.stringify(result)}`);
    ...
    const row = rows[0];
    assert(row.Status === 'approved', `Expected staged live row status approved, got ${row.Status}`);
