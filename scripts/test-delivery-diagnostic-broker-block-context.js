...
(function main() {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'delivery-broker-block-context-'));
  const portfolioDir = seedPortfolio(repoRoot);
  const result = evaluateDeliveryPosture({ portfolioDir });

  assert(result.deliveryPosture.ready === false, 'expected delivery posture not ready');
  assert(/backfill state/i.test(result.deliveryPosture.recommendedNextAction), `expected delivery-specific next action, got: ${result.deliveryPosture.recommendedNextAction}`);
  assert(result.deliveryPosture.brokerBlockContext, 'expected brokerBlockContext');
  assert.strictEqual(result.deliveryPosture.brokerBlockContext.blockedTradeCount, 1, 'expected one blocked trade');
  assert(result.deliveryPosture.brokerBlockContext.topBrokerBlock, 'expected top broker block');
  assert.strictEqual(result.deliveryPosture.brokerBlockContext.topBrokerBlock.blockCode, 'exchange_closed_at_submit');
  assert(/exchange was closed at submission time/i.test(result.deliveryPosture.brokerBlockContext.topBrokerBlock.blockReason), 'expected broker block reason');
  assert(/market-open runner/i.test(result.deliveryPosture.brokerBlockContext.topBrokerBlock.nextAction), 'expected broker block next action');

  console.log(JSON.stringify({ ok: true, deliveryPosture: result.deliveryPosture }, null, 2));
})();
