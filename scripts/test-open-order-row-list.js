...
function main() {
  ...
  assert(rows[0].tickerOrIsin === 'BBB', 'Expected submitted row included');
  assert(rows[1].action === 'sell', 'Expected partially-filled sell row included');
  assert(rows.every((row) => ['202', '303'].includes(String(row.brokerOrderId))), 'Only broker-linked submitted/partial rows should be listed');
