...
const { clearBrokerErrors, brokerErrorStatus } = require('../src/execution/runtimeState');

function assert(condition, message) {
  if (!condition) throw new Error(message);
}

function writeFixturePortfolio(portfolioDir) {
...
async function main() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cancel-runtime-error-'));
  const portfolioDir = path.join(tempDir, 'portfolio');
  writeFixturePortfolio(portfolioDir);
          ...
          async cancelOrder(orderId) {
            if (shouldFail) {
              return {
                ok: false,
                reason: 'cancel_error',
                error: `cancel failed for ${orderId}`,
              };
            }
            return {
              ok: true,
  ...
  try {
    ...
    const failed = await cancelPortfolioOrder({
      portfolioDir,
      orderId: '4242',
      selector: { orderId: '4242', tickerOrIsin: 'AAA', action: 'buy' },
      userApproved: true,
    });
    assert(!failed.ok, 'Expected first cancel attempt to fail');
    const afterFail = brokerErrorStatus(portfolioName);
    assert(afterFail.consecutive === 1, `Expected one recorded cancel failure, got ${afterFail.consecutive}`);
    assert(afterFail.lastReason === 'cancel_error', `Expected cancel_error lastReason, got ${afterFail.lastReason}`);

    shouldFail = false;
    const succeeded = await cancelPortfolioOrder({
      portfolioDir,
      orderId: '4242',
      selector: { orderId: '4242', tickerOrIsin: 'AAA', action: 'buy' },
      userApproved: true,
    });
    assert(succeeded.ok, `Expected second cancel attempt to succeed, got ${JSON.stringify(succeeded)}`);
    const afterSuccess = brokerErrorStatus(portfolioName);
    assert(afterSuccess.consecutive === 0, `Expected broker error state cleared after successful cancel, got ${afterSuccess.consecutive}`);

    console.log(JSON.stringify({ ok: true, afterFail, afterSuccess }, null, 2));
  } finally {
