...
async function main() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'cancel-order-fallback-'));
  ...
  try {
    ...
    const result = await cancelPortfolioOrder({
      portfolioDir,
      orderId: '4242',
      selector: { orderId: '4242', tickerOrIsin: 'WRONG', action: 'buy' },
      userApproved: true,
    });
    ...
    assert(rows.length === 1, `Expected fallback reconciliation without duplicate rows, got ${rows.length}`);
    const row = rows[0];
