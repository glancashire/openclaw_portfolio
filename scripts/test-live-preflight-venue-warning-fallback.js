...
function createFixture() {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'preflight-venue-fallback-'));
...
(async () => {
  ...
  const result = await mod.evaluateLiveReadinessPreflight({
    portfolioDir,
    now: new Date('2026-05-21T08:00:00Z'),
    contractDetailsByTicker: {}
  });

  assert(result.warnings.some((w) => /UBSPX|IE00BD4TXW66|IBIS/i.test(w.message)), 'expected row-level venue warning');
  assert.strictEqual(result.marketWindow.diagnostics[0].hours.liquid.sourceKind, 'reference');
  assert.strictEqual(result.marketWindow.diagnostics[0].hours.liquid.status, 'before_open');
  process.chdir(cwd);
